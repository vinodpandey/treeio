...
VIRTUAL_ENV_PATH = path.join(PROJECT_ROOT, '..')
# sensitive data
SECRETS = json.load(open(path.join(VIRTUAL_ENV_PATH, 'secrets.json')))
...
ADMINS = (
    (str(SECRETS['ADMIN_NAME']), str(SECRETS['ADMIN_EMAIL'])),
)
...
IMAP_SERVER = ''
EMAIL_USERNAME = str(SECRETS['EMAIL_USERNAME'])
EMAIL_PASSWORD = str(SECRETS['EMAIL_PASSWORD'])
EMAIL_FROM = str(SECRETS['EMAIL_FROM'])
DEFAULT_SIGNATURE = str(SECRETS['DEFAULT_SIGNATURE'])
